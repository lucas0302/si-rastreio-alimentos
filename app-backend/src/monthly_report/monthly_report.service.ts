...
export class MonthlyReportService {
  ...
  async create(createMonthlyReportDto: any /* CreateMonthlyReportDto */) {
    ...
    const data: Prisma.MonthlyShipmentReportCreateInput = {
      ...
      productionDate: new Date(createMonthlyReportDto?.productionDate ?? new Date()),
      shipmentDate: new Date(createMonthlyReportDto?.shipmentDate ?? new Date()),
  ...
  update(id: number, updateMonthlyReportDto: any /* UpdateMonthlyReportDto */) {
    ...
    if (typeof updateMonthlyReportDto?.quantity === 'number') {
      (data as any).quantity = new Prisma.Decimal(updateMonthlyReportDto.quantity);
    }
    if (typeof updateMonthlyReportDto?.temperature === 'number') {
      (data as any).temperature = new Prisma.Decimal(updateMonthlyReportDto.temperature);
    }
    if (typeof updateMonthlyReportDto?.destination === 'string') data.destination = updateMonthlyReportDto.destination;
    if (typeof updateMonthlyReportDto?.deliverer === 'string') data.deliverer = updateMonthlyReportDto.deliverer;
    if (updateMonthlyReportDto?.productionDate) data.productionDate = new Date(updateMonthlyReportDto.productionDate);
    if (updateMonthlyReportDto?.shipmentDate) data.shipmentDate = new Date(updateMonthlyReportDto.shipmentDate);
    if (typeof updateMonthlyReportDto?.productId === 'number') data.productId = updateMonthlyReportDto.productId;
    if (typeof updateMonthlyReportDto?.customersId === 'number') data.customersId = updateMonthlyReportDto.customersId;
